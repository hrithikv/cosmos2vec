...
data = pd.read_csv('all.csv',index_col = 0)         #for first iteration
corpus = []             #for every iteration 
X = data['comment']     #for first iteration only
for i in range(len(X)):
    ...
    review = review.lower()         #run this for last iteration only
    review = review.split()
    review = ' '.join(review)
...
X = clean['comment']        #from second iteration
